...
class invalid_slug_error(Exception):
    pass


class slug_dosent_exists_error(Exception):
    pass


class slug_already_exists_error(Exception):
    pass
...
async def add_link(url: str, host, slug: Optional[str] = None):
    ...
    if len(theslug) < 4 or len(theslug) > 20:
        ...
        if not check_if_slug_exists:
            ...
            raise slug_already_exists_error("the slug is already exists")


async def get_link(slug: str, host):
    check_slug_exists = await link_exists(slug=slug)
    if not check_slug_exists:
        raise slug_dosent_exists_error("the slug is not exists")
    else:
